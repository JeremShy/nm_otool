...
void	create_list_64(t_data *data, size_t poids)
{
	...
	while(i < data->nsyms)
	{
		data->list = add_elem_end(data, data->list, (uint64_t)(data->symoff + i * sizeof(struct nlist_64)), poids);
		i++;
	}
...
void	create_list_32(t_data *data, size_t poids)
{
	...
	while(i < data->nsyms)
	{
		data->list = add_elem_end(data, data->list, (uint64_t)(data->symoff + i * sizeof(struct nlist)), poids);
		i++;
	}
	data->list = ft_sort(data->list);
}
